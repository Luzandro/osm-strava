...
def transform(coords_merc, bbox_merc, pixel_size):
    ...
    for coord in coords_merc:
        transformed.append((round((coord[0]-bbox_merc[1])/pixel_size), round((bbox_merc[0]-coord[1])/pixel_size)))
    return transformed

# Transforms image coordinates to projected coordinates
def reverse_transform(coords, bbox_merc, pixel_size):
    return (x2lon(coords[1]*pixel_size+bbox_merc[1]), y2lat(bbox_merc[0]-coords[0]*pixel_size))
...
def overpass_request(lat_ul_merc, lon_ul_merc, lat_lr_merc, lon_lr_merc):
    ...
    url = "https://overpass-api.de/api/interpreter?data=" + requests.utils.quote(
        ...
        'nwr[leisure~"track|pitch|sports_centre|stadium"];'
        'nwr["piste:type"];'
        'nwr[aerialway];'
...
def check_strava_tile(polygon_area, x, y, zoom):
    (lat_ul, lon_ul, lat_lr, lon_lr) = get_geo_bbox(x, y, zoom)     # Get bounding box of strava tile in geographical coordinates
    polygon_strava = Polygon(((lon_ul,lat_ul), (lon_lr,lat_ul), (lon_lr, lat_lr), (lon_ul,lat_lr), (lon_ul, lat_ul)))   # Create a polygon for the bounding box

    if polygon_area is None or polygon_strava.intersects(polygon_area): # Checks if the tile is in the area
...
#        if debug:
#            draw.rectangle((0,0,511,511), fill=255, outline=255)    # Fill with white pixels to display the mask

        (lat_ul_merc, lon_ul_merc, lat_lr_merc, lon_lr_merc) = get_merc_bbox(x, y, zoom)    # Get bounding box of strava tile in Mercator coordinates
        ...
        osm_root = overpass_request(lat_ul_merc+distance, lon_ul_merc-distance, lat_lr_merc-distance, lon_lr_merc+distance)
        ...
        data = np.array(image)
        while np.max(data) >= threshold:    # Is the lighter pixel in the Strava tile above the threshold ?
            maximum = np.max(data)
...
#                print(f"geo:{result[1]},{result[0]}?z={zoom}")
                print_verbose(f"https://www.openstreetmap.org/?mlat={result[1]}&mlon={result[0]}#map={zoom}/{result[1]}/{result[0]}&layers=N")

                id = f"{zoom}/{x}/{y}/{max_index[0]}/{max_index[1]}"    # Unique ID for the MapRoulette task
                status = ""
                ...
                if status != "Too_Hard" and status != "Not_an_Issue":
                    ...
                    print(f'{RS}{{"type":"FeatureCollection","features":[{{"type":"Feature","geometry":{{"type":"Point","coordinates":[{result[0]}, {result[1]}]}},"properties":{{"id":"{id}","latitude":"{result[0]}","longitude":"{result[1]}","distance":"{distance}","threshold":"{threshold}","maximum":"{maximum}","threshold":"{threshold}","min_size":"{min_size}","size":"{size}"}}}}],"id":"{id}"}}', file=geojson_file)
